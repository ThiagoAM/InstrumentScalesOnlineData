...
- `education/` contains course catalogs and lesson content.
- `home/` contains the home screen "what's new" and tips payloads.
- `toggles/` contains lightweight feature flags.
...
The `education` directory is hierarchical. Each level has a catalog file, and each catalog entry points to a child directory with the same slug-like `id`.

The traversal pattern is:

1. Read `education/<tier>/courses.json`.
2. Pick a course entry such as `guitar` or `bass-free`.
3. Read `education/<tier>/courses/<course-id>/sections.json`.
...
```text
education/max/courses/guitar/sections/guitar-fretboard-foundations/units/landmarks-and-positions/lessons/low-e-string-landmarks-3rd-5th-7th-12th-frets/lesson-content.json
```
...
- `education/max/images/`

The course JSON points at fully qualified URLs such as:

```text
https://thiagoam.github.io/instrumentScales/api/education/max/images/guitar.jpg
```
...
`home/home.json` and `home/home-dev.json` provide localized home-screen content:

- `whatsNew`
- `tips`
- `updatedAt`

One detail to keep in mind: `home` uses `zh-CN`, while `education` uses `zh-Hans`. If the app shares locale mapping logic across both datasets, it should normalize those Chinese locale keys.

### `toggles/`

`toggles/feature-toggles.json` and `toggles/feature-toggles-dev.json` are simple environment-specific feature flag payloads. Right now they only expose `display-pro-version-for-everyone`.
